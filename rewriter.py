...
改寫規則：
1. 【結構不動】保持與 A 店相同的排列結構：主品項詞在前，屬性詞在中間，送禮詞分散在後半段
2. 只替換部分次要關鍵字，換成「不同維度」的詞（例：把場合詞換成風格詞，把功能詞換成對象詞）
3. 【字數要求】總字數（含空格）必須接近 {char_limit} 字（允許 ±2 字），不可太短也不可超過
4. 只輸出關鍵字，詞間空格，不加說明{cat_rule}{code_note}{cat_hint}{hist_text}
